...
function QuestionForm({ sectionId, onSaved, onCancel }) {
  ...
  const needsOptions = ['mcq_single', 'mcq_multiple', 'true_false', 'assertion_reason'].includes(
    form.question_type
  )
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <input
              type="number" className="input-field" value={form.marks}
              min={0} step={0.5} onChange={e => set('marks', e.target.value)}
            />
          </div>
          <div>
            <label className="label">Negative</label>
            <input
              type="number" className="input-field" value={form.negative_marks}
              min={0} step={0.25} onChange={e => set('negative_marks', e.target.value)}
            />
          </div>
        </div>
      </div>
      ...
      {needsOptions && (
        <div>
          <label className="label">
            Options {form.question_type === 'mcq_single' ? '(select correct)' : '(select all correct)'}
          </label>
          <div className="space-y-2">
              ...
              <div key={i} className="flex items-center gap-2">
                <input
                  ...
                  onChange={e =>
                    setOption(
                      i,
                      'is_correct',
                      form.question_type === 'mcq_multiple' ? e.target.checked : true
                    )
                  }
                  className="accent-brand-500 w-4 h-4 flex-shrink-0"
                />
                <input
                  type="text" className="input-field text-sm py-2"
                  value={opt.text}
                ...
                {form.options.length > 2 && (
                  <button
                    ...
                    className="text-red-500 hover:text-red-400 text-lg px-1"
                  >
                    ×
                  </button>
                )}
            ...
            <button onClick={addOption} className="btn-ghost text-xs text-slate-400">
              + Add option
            </button>
          </div>
        </div>
...
export default function AdminExamEditor() {
  ...
  const [addingQuestion,  setAddingQuestion]  = useState(null) // sectionId | null
  ...
  const handleAddSection = async () => {
    ...
    try {
      const res = await examAPI.addSection(examIdToUse, {
        ...
        // backend SectionCreate expects `order`, not `order_index`
  ...
  const handleDeleteSection = async (sectionId) => {
    ...
    try {
      // examAPI.deleteSection now correctly calls DELETE /api/exams/{examId}/sections/{sectionId}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">
              Fill in details, then add sections and questions.
            </p>
          </div>
          <button onClick={() => navigate('/admin/exams')} className="btn-ghost text-sm">
            ← Back
          </button>
        </div>
        ...
        <div className="glass-card p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Final Semester Exam — Mathematics"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Brief overview visible to candidates"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Full instructions shown before the exam starts..."
            />
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              ...
              <input
                type="number" className="input-field" value={form.duration_minutes}
                min={5} onChange={e => setField('duration_minutes', Number(e.target.value))}
              />
            </div>
            <div>
              <label className="label">Total Marks</label>
              <input
                type="number" className="input-field" value={form.total_marks}
                min={1} onChange={e => setField('total_marks', Number(e.target.value))}
              />
            </div>
            <div>
              <label className="label">Pass %</label>
              <input
                type="number" className="input-field" value={form.pass_percentage}
                min={0} max={100} onChange={e => setField('pass_percentage', Number(e.target.value))}
              />
            </div>
            <div>
              <label className="label">Max Attempts</label>
              <input
                type="number" className="input-field" value={form.max_attempts}
                min={1} onChange={e => setField('max_attempts', Number(e.target.value))}
              />
            </div>
          </div>
            ...
            {sections.map(section => (
              <div key={section.id} className="glass-card p-6 space-y-4">
                ...
                {(questions[section.id] || []).map((q, i) => (
                  <div
                    key={q.id}
                    className="flex items-start justify-between bg-slate-800/40 rounded-xl p-4 gap-3"
                  >
                ...
                {addingQuestion === section.id ? (
                  ...
                  <button
                    className="btn-ghost text-sm text-slate-400 border border-dashed border-slate-700
                      w-full justify-center py-3 rounded-xl hover:border-brand-500/50 hover:text-brand-400"
