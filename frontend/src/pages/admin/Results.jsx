...
export default function AdminResults() {
  ...
  const handlePublish = async (resultId) => {
    ...
    try {
      ...
      setResults(rs => rs.map(r => r.id === resultId ? { ...r, is_published: true } : r))
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">{results.length} result{results.length !== 1 ? 's' : ''}</p>
          </div>
          <select
        ...
        {loading ? (
          ...
          <div className="glass-card p-16 text-center">
            ...
            <p className="text-slate-400">Results will appear here after candidates submit exams.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-800">
                  {['Candidate', 'Exam', 'Score', 'Status', 'Date', 'Published', 'Action'].map(h => (
              ...
              <tbody className="divide-y divide-slate-800">
                {results.map(result => {
                  const scorePercent = result.total_marks > 0
                    ? Math.round((result.obtained_marks / result.total_marks) * 100) : 0
                  return (
                    <tr key={result.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="px-4 py-4">
                        <p className="font-medium text-white text-sm">{result.candidate_name || '—'}</p>
                        <p className="text-slate-500 text-xs">{result.candidate_email || ''}</p>
                      </td>
                      <td className="px-4 py-4 text-slate-300 text-sm">{result.exam_title || `Exam #${result.exam_id}`}</td>
                      <td className="px-4 py-4">
                        ...
                        <p className="text-slate-500 text-xs mt-0.5">{result.obtained_marks}/{result.total_marks}</p>
                      </td>
                      <td className="px-4 py-4">
                        <span className={STATUS_BADGE[result.status] || 'badge-gray'}>
                          {result.status?.replace('_', ' ')}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-slate-500 text-xs">{formatDate(result.created_at)}</td>
                      <td className="px-4 py-4">
                        <span className={result.is_published ? 'badge-green' : 'badge-gray'}>
